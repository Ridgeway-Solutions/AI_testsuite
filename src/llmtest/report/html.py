...
def render_html(result: RunResult) -> str:
    ...
    w(f"<!doctype html><html><head><meta charset='utf-8'>"
      ...
      f"<title>Adversarial robustness report — {_e(result.config.name)}</title><style>{CSS}</style>"
    ...
    w("<footer>Generated by llm-testsuite. Findings show a reachable boundary in "
